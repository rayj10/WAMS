  ...
  */
  renderPage(pageName) {
    ...
    if (this.props.taskListReceived)
      return <SummaryListPage name={pageName} title={'Task List'} status={this.state.fetchStatus} onRefresh={refresh} list={this.props.taskList} keys={keys} onShowDetails={(reqHead) => Actions.DODetails({ header: reqHead, user: 'Installer', keys, refresh })} />
    else
      return <SummaryListPage name={pageName} title={'Task List'} status={this.state.fetchStatus} onRefresh={refresh} />
  }

...
class DODetails extends React.Component {
    ...
    render() {
        let editInstallerButton = null;

        if (this.props.user === 'Admin')
            editInstallerButton = <IconWrapper name='person-pin-circle' color='white' size={28} style={styles.icon} onPress={() => this.setState({ editInstaller: true })} />;

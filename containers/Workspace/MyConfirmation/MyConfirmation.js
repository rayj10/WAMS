  ...
  */
  renderPage(pageName) {
    ...
    if (this.props.requestConfirmationReceived)
      return <SummaryListPage name={pageName} title={'My Confirmation'} status={this.state.fetchStatus} onRefresh={refresh} list={this.props.requestConfirmationList} keys={keys} onShowDetails={(reqHead) => Actions.RequestConfirm({ header: reqHead, keys, refresh })} />
    else
      return <SummaryListPage name={pageName} title={'My Confirmation'} status={this.state.fetchStatus} onRefresh={refresh} />
  }
